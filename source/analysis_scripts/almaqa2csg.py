...
def doGenerateTsysCalTable(msName, calTableName=[], doplot=True, isB2B=False):
    ...
    for i in sorted(tsysSpwInfo.keys()):
        if tsysSpwInfo[i]['numChans'] <= 256:
            ...
            spwSpec = spwSpec+str(i)+':0~'+str(int(tsysSpwInfo[i]['numChans']*chanEdge1-1))+';'+str(int(tsysSpwInfo[i]['numChans']-tsysSpwInfo[i]['numChans']*chanEdge1))+'~'+str(tsysSpwInfo[i]['numChans']-1)
...
def doInitialFlagging(msName, msName1='', chanEdge=0.0625, thresh=0.2, iHaveSplitMyScienceSpw=False):
    ...
    for i in range(len(spwIds)):
        if spwIds[i] in TDMspws:
            ...
            spwSpec = spwSpec+':0~'+str(int(sciNumChans[i]*chanEdge-1))+';'+str(int(sciNumChans[i]-sciNumChans[i]*chanEdge))+'~'+str(sciNumChans[i]-1)
